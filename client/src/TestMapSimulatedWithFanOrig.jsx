...
//const backendUrl = import.meta.env.VITE_BACKEND_LIVE_DEV_URL || import.meta.env.VITE_BACKEND_LIVE_DEV_URL;
...
/// this we will work later .. we need custoemr address to auto fetch. 
  ...
  const resetCar = () => {
    ...
    updateTripCost(selectedTrip._id, -prev.cost + 1000); // Reset to flat rate
  };
  ...
  const updateTripStatus = async (tripId, status) => {
    ...
    //   return; // Skip update if the status is already the same
    // }
  ...
  // if (trip.distance === distance && trip.duration === duration && trip.cost === cost) {
  //   return;
  // }
    try {
      ...
      {selectedOption && (
        <Draggable disabled={isPinned}>
          <div className="absolute top-20 left-20 z-30 bg-white border shadow-lg rounded-md p-4 w-64">
            ...
            {selectedOption === "car" && (
              <>
                <p><strong>Model:</strong> {selectedTrip?.vehicle || "N/A"}</p>
                <p><strong>License Plate:</strong> {selectedTrip?.licensePlate || "N/A"}</p>
